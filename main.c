		...
		{ 
			...
			if (highlight == 0) 
			{
				...
				while(1)
				{
					...
					WINDOW* addDeleteMenu = newwin(y-8,x-4,5,2);
					box(addDeleteMenu,0,0);
					refresh();
					wrefresh(addDeleteMenu);
					if(highlight == 0)//Add
					{
						char searchWord[50];
						char c;
						...
						mvwprintw(addDeleteMenu,1,1, "Type one word to search by:");
						wgetstr(addDeleteMenu, searchWord);
						AVLNODE* searched = search(tree, searchWord);
						printList(searched->list, addDeleteMenu);
						mvwprintw(addDeleteMenu,y-20,1, "Enter full name of the movie you wish to add:");
						mvwprintw(addDeleteMenu,y-19,1, "No Backspaces!!");
						while((c = wgetch(addDeleteMenu)) != '\n')
						{
							word[stringIterator] = c;
							word[stringIterator + 1] = '\0';
							c = wgetch(addDeleteMenu);
							stringIterator++;
						}
						userLog->recentAdded = addToLog(userLog->recentAdded, word);
						refresh();
					}
					else
					{
